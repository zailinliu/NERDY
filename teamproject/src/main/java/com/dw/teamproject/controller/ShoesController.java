...
private FunctionService functionService;
	...
	@Autowired
	public ShoesController(FunctionService functionService) {
		super();
	...
	@PostMapping
//	사이트 생성
	public ResponseEntity<Shoes> saveshoes(
			@RequestBody Shoes shoes){
		return new ResponseEntity<Shoes>(
				functionService.saveShoes(shoes),
				HttpStatus.CREATED
				);
	}
...
//		http 요청 또는 응답에 해당하는 컴포넌트를 포함한 리스폰스엔티티를 사용 배열안에 게임을 넣어 순차적으로 보이게 하기 위함
		List<Shoes> shoes = functionService.getAllShoes();
		return new ResponseEntity<>(shoes,HttpStatus.OK);
	}
	
	@GetMapping("{id}")
	public ResponseEntity<Shoes> getshoesById(
			@PathVariable("id") long id){
//			주소 뒤에 붙힐 id 값을 무슨 형식으로 넣을지 정하는 부분
	return new ResponseEntity<Shoes>(
			functionService.getShoesById(id),HttpStatus.OK);
	}
	
	@PutMapping("{id}")
//	정보를 입력할때 사용하는 풋매핑
	public ResponseEntity<Shoes> updateshoesById(
			@RequestBody Shoes shoes, @PathVariable long id)
//			비동기방식(화면전환 없이 이루어지는 동작)을 사용하기 위해 먼저 요청 그후 응답 형식으로 이루어짐
			{
		return new ResponseEntity<Shoes>(
				functionService.updateShoesById(shoes, id),
				HttpStatus.OK);
	}
	
	@DeleteMapping("{id}")
//	게임 삭제
	public ResponseEntity<String> deleteShoesById(@PathVariable long id){
		functionService.deleteShoes(id);
		return new ResponseEntity<String>("shoes deletd successfully",HttpStatus.OK);
	}
